...
public:
	...
	ListSequence<TItem>* findNode(Node* Node1, TKey key, ListSequence<TItem>* List) {
		if (Node1 == nullptr) {
		...
		if (Node1->key == key) {
			List->Prepend(Node1->item);
			if (Node1->right != nullptr) {
				this->findNode(Node1->right, key, List);
			}
			if (Node1->left != nullptr) {
				this->findNode(Node1->left, key, List);
			}
		}
		else {
			if (Node1->key < key) {
				return findNode(Node1->right, key, List);
			}
			else {
				return findNode(Node1->left, key, List);
			}
		}
		return List;
	};
	...
	ListSequence<TItem>* findNode(TKey key) {
		ListSequence<TItem>* List = new ListSequence<TItem>();
		findNode(this->root, key, List);
		return List;
	};
